...
class ReqGen:
    ...
    def gen(self):
        req_infos = []
        service_tuple_array = getattr(
            getattr(
                getattr(self.modules.pb2, 'DESCRIPTOR')
                ,'services_by_name')
            ,'items')()
        for service in service_tuple_array:
            # first item in tuple is the name, protoc adds stub to the name
            stub_name = service[0]+"Stub"
            # second item in tuple is the ServiceDescriptor class .methods are requests like
            # SayHello, input_type is like HelloRequest proto
            for request in service[1].methods: 
